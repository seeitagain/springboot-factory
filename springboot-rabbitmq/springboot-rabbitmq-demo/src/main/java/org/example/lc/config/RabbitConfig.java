...
import org.example.lc.message.Demo01Message;
import org.example.lc.message.Demo02Message;
    ...
    public static class DirectExchangeDemoConfiguration {
        ...
        public Queue demo01Queue() {
            return new Queue(Demo01Message.QUEUE, // Queue 名字
                    true, // durable: 是否持久化 队列为持久化队列，重启 RabbitMQ 后依然存在
                    false, // exclusive: 是否排它 非排他队列，多个连接可以访问
                    false); // autoDelete: 是否自动删除 没有消费者时不会自动删除该队列
        }
